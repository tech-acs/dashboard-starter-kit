...
class CreateMapIndicator extends Tool
{
    ...
    public function handle(Request $request, CreateArtefactAction $createArtefactAction): Response
    {
        ...
        if ($result->success) {
            return Response::text("Map indicator '{$result->artefact->name}' created successfully at {$result->filePath}. Use this full name (including the data source prefix) for all subsequent tools (validate-artefact, edit-map-indicator).");
        }
    ...
    {
        return [
            'name' => $schema->string()->description('Component name in CamelCase without the data source prefix (e.g., "MyMapIndicator", "CompletionMap"). The data source title is automatically prepended as a directory (e.g., "Households/MyMapIndicator"). Do NOT include the data source folder yourself.'),
            'title' => $schema->string()->description('Human-readable title'),
            'description' => $schema->string()->description('Human-readable description'),
            'data_source' => $schema->string()->description('Name of the data source this map indicator queries (use the `name` field from get-data-sources, e.g. "households")'),
        ];
    }
}
