...
describe('CreateArtefactAction (gauge)', function () {
    beforeEach(function () {
    ...
    it('creates gauge and returns success result', function () {
        ...
        $kernelMock->shouldReceive('call')
            ...
            ->with('chimera:make-artefact', Mockery::on(function (array $params) {
                return $params['name'] === 'TestGauge'
                    && $params['--namespace'] === 'Livewire\Gauge';
            }))
            ->andReturn(Command::SUCCESS);
        Artisan::swap($kernelMock);

        $action = app(CreateArtefactAction::class);
        $result = $action->execute(modelClass: Gauge::class, baseNamespace: 'Livewire\Gauge', attributes: new GaugeAttributes(
            name: 'TestGauge',
            title: 'Test Title',
        ...
        expect($result->filePath)->toContain('TestGauge.php');
    });
...
describe('Web Path', function () {
    ...
    it('creates gauge via http post and redirects with success', function () {
        $this->mockAction
            ->shouldReceive('execute')
            ->once()
            ->andReturn(ArtefactCreationResult::success(
                fakeGaugeModel(),
                app_path('Livewire/Gauge/TestGauge.php'),
            ));

        $response = $this
            ->withoutMiddleware()
            ->post(route('developer.gauge.store'), [
                'name' => 'TestGauge',
                'title' => 'Test Title',
                'subtitle' => 'Test Subtitle',
        ...
        $response->assertRedirect(route('gauge.index'));
        $response->assertSessionHas('message', 'Gauge created');
    });
...
describe('CLI Path', function () {
    ...
    it('creates gauge and returns success exit code', function () {
        $this->mockAction
            ->shouldReceive('execute')
            ->once()
            ->andReturn(ArtefactCreationResult::success(
        ...
        $this->artisan('chimera:make-gauge')
            ->expectsChoice(
                'Which data source will this gauge be using?',
                'households',
                ['households' => 'Households'],
            )
            ->expectsQuestion('Gauge name', 'TestGauge')
            ->expectsQuestion('Please enter a reader friendly title for the gauge', 'Test Title')
            ->expectsQuestion('Please enter a reader friendly sub-title for the gauge', 'Test Subtitle')
            ->assertSuccessful();
    });
...
describe('MCP Path', function () {
    ...
    it('creates gauge and returns success response', function () {
        $this->mockAction
            ->shouldReceive('execute')
            ->once()
            ->andReturn(ArtefactCreationResult::success(
